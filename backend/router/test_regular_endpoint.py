#!/usr/bin/env python3
"""
Test script for the /api/stream endpoint
Tests if the regular chat works properly
"""
...
STREAM_ENDPOINT = f"{API_URL}/api/stream"
...
async def test_regular_endpoint():
    ...
    log_with_timestamp("🧪 Starting regular endpoint test")

    # Test message
    test_message = "Hello, tell me about AI"
    ...
    log_with_timestamp(f"📤 Sending request to {STREAM_ENDPOINT}")
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            async with client.stream(
                ...
                if orchestrator_start_received and orchestrator_complete_received and token_count > 0:
                    log_with_timestamp("✅ SUCCESS: Regular chat completed properly!")
                    return True
                else:
...
async def main():
    ...
    success = await test_regular_endpoint()
