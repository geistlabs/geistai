...
export default function StorageScreen() {
  const [activeTab, setActiveTab] = useState<'embeddings' | 'memories'>(
    'embeddings',
  );
  ...
  const loadMemories = useCallback(async () => {
    console.log('[Storage] 🧠 Loading memories...');
    console.log(
      '[Storage] 🧠 Memory manager initialized:',
      memoryManager.isInitialized,
    );

    if (!memoryManager.isInitialized) {
      console.log(
        '[Storage] 🧠 ❌ Memory manager not initialized, skipping load',
      );
      return;
    }
    ...
    try {
      setIsLoading(true);
      console.log('[Storage] 🧠 Fetching memories from database...');
      const [allMemories, stats] = await Promise.all([
        memoryStorage.getAllMemories(), // Get all memories
        memoryManager.getMemoryStats(),
      ]);
      console.log('[Storage] 🧠 Loaded memories count:', allMemories.length);
      console.log('[Storage] 🧠 Memory stats:', stats);
      setMemories(allMemories);
      setMemoryStats(stats);
    } catch (error) {
      console.error('[Storage] 🧠 ❌ Failed to load memories:', error);
      Alert.alert('Error', 'Failed to load memories');
    } finally {
      setIsLoading(false);
    }
  }, [memoryManager.isInitialized]); // Add memoryManager.isInitialized as dependency
  ...
  useEffect(() => {
    console.log('[Storage] 🧠 Tab changed to:', activeTab);
  ...
  const forceRefreshMemories = async () => {
    console.log('[Storage] 🧠 Force refreshing memories...');
    await loadMemories();
  };
        ...
        <View className='border-b border-gray-200 bg-white'>
          ...
          <View className='flex-row border-t border-gray-200'>
            ...
            >
              <Text
                className={`text-center font-medium ${
                  activeTab === 'embeddings' ? 'text-blue-600' : 'text-gray-600'
                }`}
              >
            ...
            >
              <Text
                className={`text-center font-medium ${
                  activeTab === 'memories' ? 'text-blue-600' : 'text-gray-600'
                }`}
              >
          ...
          <View className='space-y-3'>
            ...
            <TouchableOpacity
              ...
              className={`rounded-lg px-4 py-3 ${
                isEmbedding ? 'bg-gray-400' : 'bg-blue-600'
              }`}
            >
              ...
              >
                <View className='flex-row items-center justify-center'>
                  ...
                  <Text className='text-center text-base font-medium text-white'>
                    {isSearching
                      ? 'Searching...'
                      : `Search ${activeTab === 'embeddings' ? 'Similar' : 'Memories'}`}
                  </Text>
                </View>
              </TouchableOpacity>
              <TouchableOpacity
        ...
        <View className='flex-1 p-4'>
          <View className='mb-3 flex-row items-center justify-between'>
            ...
            <View className='flex-row space-x-2'>
              <TouchableOpacity
                onPress={
                  activeTab === 'embeddings' ? loadStorageItems : loadMemories
                }
          ...
          {isLoading ? (
            <View className='flex-1 items-center justify-center'>
              ...
              <Text className='mt-2 text-gray-500'>Loading {activeTab}...</Text>
            </View>
          ) : (
              activeTab === 'embeddings'
            ...
            <FlatList
              ...
              keyExtractor={item =>
                activeTab === 'embeddings' ? item.id.toString() : item.id
              }
