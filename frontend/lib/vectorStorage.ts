...
class VectorStorageService {
  ...
  async getAllEmbeddings(): Promise<VectorEmbedding[]> {
    ...
    const rows = await this.db!.getAllAsync('SELECT * FROM embeddings ORDER BY created_at DESC');
  ...
  async searchSimilar(queryEmbedding: number[], limit: number = 10): Promise<SimilarityResult[]> {
    if (!this.db) await this.initDatabase();
